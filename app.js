...
function taskOrganization(expert, task) {
  const override = data.taskOrganizationOverrides?.[`${task.sessionId}|${expert.name}`];
  const text = validText(override || expert.organization);
...
function renderTaskDetail() {
  ...
  const taskItems = tasks.map((task) => {
    ...
    const organizationLine = taskTextLine("工作单位", taskOrganization(expert, task));
    return `
...
function copyExpertTasks(expert, tasks) {
  ...
  sortedTasks(tasks).forEach((task, index) => {
    ...
    const organization = taskOrganization(expert, task);
